...
export class DestinationSwitchAccessory {
  ...
  async setOn(value: CharacteristicValue) {
    ...
    if (value as boolean) {
      ...
      const destination = { destinations: [{ poiProvider: 'unknown', destinationName: this.accessory.context.device.name, address: this.accessory.context.device.address, destinationSource: 'MobileApp', geoCoordinate: { longitude: this.accessory.context.device.lon, latitude: this.accessory.context.device.lat } }] };
